  ...
  return (
    <ForumShell currentSection="questions">
      <section className="p-5">
        ...
        <p className="mt-4 text-sm text-[var(--jr-text-muted)]">Browse all questions in the community.</p>
      </section>
      ...
      <section>
        {forumQuestions.map((question) => (
          <article key={question.id} className="p-5">
            ...
            <p className="mt-4 text-sm text-[var(--jr-text-muted)]">by {formatUsername(question.author)}</p>
          </article>
        ))}
      </section>
    </ForumShell>
