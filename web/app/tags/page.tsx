  ...
  return (
    <ForumShell currentSection="tags">
      <section className="px-4 py-4">
        ...
        <p className="mt-1 text-sm text-[var(--jr-text-muted)]">Explore topics used across questions.</p>
      </section>

      <section className="px-4 pb-4">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          {tags.map((tag) => (
            <div key={tag.name} className="rounded-[8px] bg-[var(--jr-surface-alt)] p-3">
              <Link href="/questions" className="text-sm font-semibold text-[var(--jr-brand)] hover:underline">
                #{tag.name}
              </Link>
              <p className="mt-1 text-sm text-[var(--jr-text-muted)]">{tag.count} questions</p>
            </div>
          ))}
        </div>
      </section>
