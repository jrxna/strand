...
export default async function UserProfilePage({ params }: UserProfilePageProps) {
  ...
  return (
    <ForumShell currentSection="users">
      <section className="border-b-[0.05px] p-5">
        <h1 className="text-xl font-bold">{formatUsername(profile.username)}</h1>
        <p className="mt-4 text-sm text-[var(--jr-text-muted)]">{profile.role}</p>
      ...
      <section className="border-b-[0.05px] p-5">
        ...
        <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
          <div className="rounded-[8px] bg-[var(--jr-surface-alt)] p-5">
            <p className="text-[var(--jr-text-muted)]">Questions</p>
            <p className="mt-4 text-xl font-semibold">{activity.questions.length}</p>
          </div>
          <div className="rounded-[8px] bg-[var(--jr-surface-alt)] p-5">
            <p className="text-[var(--jr-text-muted)]">Answers</p>
            <p className="mt-4 text-xl font-semibold">{activity.comments.length}</p>
          </div>
          <div className="rounded-[8px] bg-[var(--jr-surface-alt)] p-5">
            <p className="text-[var(--jr-text-muted)]">Replies</p>
            <p className="mt-4 text-xl font-semibold">{activity.replies.length}</p>
          </div>
        </div>
      </section>
          ...
          {activity.questions.length ? (
            activity.questions.map((question) => (
              ...
              >
                ...
                <p className="mt-4 text-sm text-[var(--jr-text-muted)]">{formatDateOnly(question.createdAt)}</p>
              </article>
            ))
          ) : (
